...
interface GrupoUserProfileDropdownProps {
...
const GrupoUserProfileDropdown: React.FC<GrupoUserProfileDropdownProps> = ({
  user,
  onEditProfile,
  onChangePassword,
  onLogout,
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <img src={user.responsiblePhotoUrl} alt={user.responsibleName} className="w-8 h-8 rounded-full" />
        <span className="hidden sm:inline font-bold text-dark-text">{user.responsibleName}</span>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-64 bg-dark-card border border-dark-border rounded-xl shadow-2xl z-30 animate-fade-in p-2">
          <div className="p-3 border-b border-dark-border">
              <p className="font-bold text-dark-text truncate">{user.name}</p>
              <p className="text-sm text-dark-secondary truncate">{user.accessEmail}</p>
          </div>
          <div className="py-2">
...
export default GrupoUserProfileDropdown;